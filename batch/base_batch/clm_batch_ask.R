...
all_day_h<-clm(ask_order~bid_vol+ask_vol+actual_spread+wait,data=df,link = "probit", Hess=T)
...
one_day_h<-df%>%
  dplyr::group_by(v2)%>%
  tidyr::nest()%>%
  dplyr::mutate(model=purrr::map(data, ~clm(ask_order~bid_vol+ask_vol+actual_spread+wait,data=.,link = "probit", Hess=T)))
...
##全部の日を使う
all_day_30_minutes_h<-clm(ask_order~bid_vol+ask_vol+actual_spread+wait,data=first_30_minutes,link = "probit", Hess=T)
