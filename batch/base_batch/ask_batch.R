...
#データの読み込み
d<-fread(data,
         select=c("row_num", "v2", "v6", "v5", "ask_vol", "bid_vol", "actual_spread", "wait", "ask_order", "time_flag"))

df<-d%>%
  dplyr::select(v2, ask_order, bid_vol, ask_vol, actual_spread, wait, time_flag)%>%
  dplyr::filter(v2>=20160928)

order<-data.table(c("1", "2", "3", "4", "5"))
count_order<- data.table(table(df$ask_order))
...
time_order_cnt<-df%>%
  dplyr::mutate(time_order=time_flag*10+ask_order)
...
df$ask_order<-factor(df$ask_order,levels=c("5","4","3","2","1"),ordered=TRUE)

#全部の時間
##全部の日を使う
all_day_h<-polr(ask_order~bid_vol+ask_vol+actual_spread+wait,data=df,method="probit",start=c(rep(0,4),rep(1:4)), Hess=T)
...
cnt_order_one_day<-df%>%
  dplyr::select(v2, ask_order, bid_vol, ask_vol, actual_spread, wait, time_flag)%>%
  dplyr::filter(v2>=20160928)%>%
  dplyr::mutate(ask_order = as.character(ask_order))%>%
  dplyr::group_by(v2, ask_order)%>%
  dplyr::mutate(cnt_order_day = n())%>%
  dplyr::ungroup()%>%
  dplyr::select(v2, ask_order, cnt_order_day)%>%
  dplyr::distinct()

cnt_order_one_day_table<-dplyr::left_join(day_order, cnt_order_one_day, by=c("Day"="v2", "V1"="ask_order"))%>%
    tidyr::spread(key = V1, value = cnt_order_day)

cnt_order_time<-df%>%
  dplyr::group_by(v2, ask_order, time_flag)%>%
  dplyr::mutate(cnt_order_time = n())%>%
  dplyr::ungroup()%>%
  dplyr::mutate(toc = paste(as.character(time_flag), as.character(ask_order), sep = ""))%>%
...
one_day_h<-df%>%
  dplyr::group_by(v2)%>%
  tidyr::nest()%>%
  dplyr::mutate(model=purrr::map(data, ~polr(ask_order~bid_vol+ask_vol+actual_spread+wait,data=.,method="probit",start=c(rep(0,4),rep(1:4)))))
...
count_order_30_minutes<- data.table(table(first_30_minutes$ask_order))
count_order_30_minutes_table<-dplyr::left_join(order, count_order_30_minutes, by="V1")

first_30_minutes$ask_order<-factor(first_30_minutes$ask_order,levels=c("5","4","3","2","1"),ordered=TRUE)

##全部の日を使う
all_day_30_minutes_h<-polr(ask_order~bid_vol+ask_vol+actual_spread+wait,data=first_30_minutes,method="probit",start=c(rep(0,4),rep(1:4)), Hess=T)
...
cnt_order_one_day_30_minutes_table<-dplyr::left_join(day_order, cnt_order_one_day_30_minutes, by=c("Day"="v2", "V1"="ask_order"))%>%
    tidyr::spread(key = V1, value = cnt_order_day)
